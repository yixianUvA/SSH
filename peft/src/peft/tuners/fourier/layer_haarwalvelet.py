...
class Linear(nn.Module, FourierLayer):
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        #weight = self.idwt2(dense_s.to_dense()) * 300

        if cast_to_fp32:
            weight = weight.float()

        #output_tensor = weight
        output_tensor = weight.squeeze(0).squeeze(0)  # Remove batch and channel dimensions
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                dense_s = dense_s.unsqueeze(0).unsqueeze(0)  # Adding batch and channel dimensions
                Yl, Yh = self.dwt(dense_s)
                delta_w = self.idwt2(Yl, Yh) * scale
                delta_w = delta_w.squeeze(0).squeeze(0)  # Remove batch and channel dimensions
