...
class FourierLayer(BaseTunerLayer):
    ...
    def idwt2(self, x):
        # Ensure the input is properly shaped (add batch and channel if necessary)
        if x.dim() == 2:  # If the input is 2D, add batch and channel dimensions
            x = x.unsqueeze(0).unsqueeze(0)

        #print(f"the shape of x is {x.shape}")  # Should print [1, 1, 768, 768]

        # Ensure the number of channels is divisible by 4 (if necessary)
        ...
        # Apply the inverse Haar transform
        #print(f'the shape of changed x is {x.shape}')
        x_idwt = self.idwt(x)

        # Check if the output shape has doubled the spatial dimensions
        if x_idwt.shape[2] > 768 or x_idwt.shape[3] > 768:
            # Slice to reduce the size back to [768, 768]
            x_idwt = x_idwt[:, :, :768, :768]

        # If the output has 4 channels (due to reconstruction), select only the approximation (LL)
        if x_idwt.shape[1] == 4:
            x_idwt = x_idwt[:, 0, :, :]  # Keep only the first channel (LL)

        # Return the result, removing the added batch and channel dimensions
        return x_idwt.squeeze(0).squeeze(0)
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                delta_w = self.idwt2(dense_s)
                #print(f"dense_s's shape is {dense_s.shape}")
                #print(f"delta_w's shape is {delta_w.shape}")
                delta_w = delta_w / 10
