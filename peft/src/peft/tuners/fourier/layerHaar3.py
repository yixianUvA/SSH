...
class FourierLayer(BaseTunerLayer):
    ...
    def idwt2(self, coeffs):
        LL, (LH, HL, HH) = coeffs
        # Ensure the input tensors are detached from the computation graph and moved to CPU before converting to NumPy
        coeffs_np = (
            LL.detach().cpu().numpy(), 
            (
                LH.detach().cpu().numpy(), 
        ...
        # Apply inverse 2D DWT and return the result as a tensor
        return torch.tensor(pywt.idwt2(coeffs_np, 'haar'), dtype=LL.dtype, device=LL.device)
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                # Create the tuple of coefficients (LL, (LH, HL, HH))
                coeffs = (LL, (LH, HL, HH))
                delta_w = self.idwt2(coeffs)
                delta_w = delta_w / 10
