...
class FourierLayer(BaseTunerLayer):
    ...
    def __init__(self, base_layer: nn.Module, **kwargs) -> None:
        ...
        if isinstance(base_layer, nn.Linear):
            ...
            in_features, out_features = base_layer.infeatures, out_features.outfeatures
    ...
    # Implement the 2D Discrete Hartley Transform (DHT) using matrix operations
    def dht2(self, x):
        """
        Compute the 2D Discrete Hartley Transform (DHT) using matrix operations.
        """
        N = x.size(-1)
        k = torch.arange(N, device=x.device)
        n = k.view(-1, 1)
        
        dht_kernel = torch.cos(2 * math.pi * k * n / N) + torch.sin(2 * math.pi * k * n / N)
        return torch.matmul(torch.matmul(dht_kernel, x), dht_kernel)

    def idht2(self, X):
        """
        Compute the inverse 2D Discrete Hartley Transform (IDHT) as it is the same as the forward DHT.
        """
        return self.dht2(X) / X.size(-1)
