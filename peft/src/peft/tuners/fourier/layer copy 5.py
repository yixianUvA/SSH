...
class FourierLayer(BaseTunerLayer):
    ...
    def dht1(self, x, norm=None):
        ...
        #H = torch.nn.functional.interpolate(H.unsqueeze(1), size=N, mode='linear').squeeze(1)
        H = self.pad_rfft_output(H, N)
        if norm == 'ortho':
            H = H / math.sqrt(N)
        else:
            H = H / N
    ...
    def pad_rfft_output(self, H, original_size):
        # Pad the rFFT result to match the original size
        padded_H = torch.nn.functional.pad(H, (0, original_size - H.size(-1)))
        return padded_H
